...
| 15 | Deauth/disassoc frame *construction* lives in the pure `deauth` core, compiled and unit-tested on the native lane; the `RawTransmitter` seam forwards fully-formed frame bytes only and carries no frame-building logic (the TX mirror of #8's read-side purity and #9's raw-bytes RX seam). The raw-TX *capability* — the device `esp_wifi_80211_tx` path and the `ieee80211_raw_frame_sanity_check` SDK bypass — is absent from any build without `SAPPER_TEST_HOOKS`: no shipped binary can transmit a deauth frame. This holds until the allowlist-gated unattended-deauth operating mode ships and *supersedes* it (a new ADR that states why loosening the gate is safe then). | ADR-0027 | review-only |
