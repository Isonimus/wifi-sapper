...
| 3 | Every state that blocks before the engine loop runs (e.g. first-boot captive portal) pumps the serial channel. The bounded ≤3 s Maintenance-entry poll window (ADR-0053) is *not* such a state — a fixed boot dwell, not an interactive/indefinite blocking phase — so, like the splash present and the pre-existing single sample it replaced, it does not pump serial (recorded here so the scope is greppable at the row, not only in ADR prose). | ADR-0003, ADR-0053 | verified_by: scripts/0007-provisioning-verify.mjs; review-only (the entry-window scope carve-out) |
...
| 21 | The Maintenance boot phase is a **hunt-suspended** phase entered only from a clean boot — a **debounced BOOT press during a bounded post-boot window** (`maintenanceRequested()` polling `bootButtonPin` *after* a normal boot, driving the ADR-0006 #3 boot gate via its repurposed third input) on a *provisioned* device — never held through reset (GPIO0 strapping would enter ROM download mode) and never a live radio reconfiguration, and left only by rebooting into Station (a power-cycle without pressing BOOT, or the 30-min no-activity backstop). The entry gesture is ADR-0053, superseding ADR-0039 decision 2's impossible hold-at-power-on; the window runs only when provisioned (`hasCreds && maintenanceRequested()`). Its dashboard reads **persisted** state only — recovered results through the `CrackedManifest`/`CrackedStore` seam (#12, via the read-only `entryAt`), the record through `loadProvisioning()` (#6), the capture depth through the `CaptureStore` — never the `EventBus` (there are no live facts in a suspended phase; corrects ADR-0021's "one more sink" expectation), and is **streamed** by the pure `net/dashboard` functions (`buildDashboardHead`/`appendCrackedRow`/`kDashboardFoot`) so the up-to-256-entry page is never materialised whole. It **deliberately serves recovered plaintext PSKs** — a recorded exception to the no-PSK doctrine of #14/#19/#20 (a local viewer on the operator's own SoftAP behind a physical BOOT press, not an off-device payload; ADR-0039 decision 5) — so the Maintenance SoftAP is hardened by an optional `ProvisioningRecord.maintenancePass` (WPA2-length or empty-for-default, set in the setup form accepted-but-never-echoed like the key, #20). The on-device panel shows connection info + a count only, never PSKs (the panel is shoulder-surfable; ADR-0039 decision 11). Extends ADR-0006 and ADR-0021; the entry gesture supersedes ADR-0039 decision 2 (ADR-0053), the rest supersedes neither. | ADR-0039, ADR-0053 | verified_by: test/test_provisioning (Maintenance gate + `isUsableMaintenancePass` + label), test/test_dashboard (streamed render incl. PSK + HTML-escape + per-piece fail-loud + backstop), test/test_provisioning_store (`maintenancePass` round-trip + absent default), test/test_provisioning_form (`maintpass` no-echo/keep), test/test_cracked_manifest (`entryAt`), test/test_entry_gesture (debounce: a glitch never enters, N consecutive LOWs do — ADR-0053); scripts/0040-maintenance-verify.mjs (on-device entry/AP/serve, forced via `SAPPER_TEST_MAINT`); review-only (`main.cpp` gate + windowed-poll wiring, `MaintenancePortal` serve, panel draw; the physical windowed press cannot be driven headlessly) |
