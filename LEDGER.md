...
- [feature] capture: target selection — slice-0014 ships band-sweep AP discovery (ApRegistry, ADR-0013); choosing which discovered AP to hunt and retuning the radio to its channel belongs to the HuntEngine (slice-4), which also owns the promiscuous-vs-STA radio arbitration (ADR-0013)
...
- [audit] RadioSniffer::stop() cannot hard-join an RX callback already in flight — blocking in the Wi-Fi-driver callback is forbidden (§4 invariant #10) — so a frame dequeued just before stop() may still write the collector after stop() returns; the engine that owns start/stop (slice-4) must sequence its read-out (quiesce or double-buffer) rather than assume quiescence (ADR-0011)
- [audit] ApRegistry::reset() must be sequenced against onFrame by the owner — onFrame runs in the Wi-Fi-driver task; the append-only, first-sighting-wins table makes count()/at() reads safe without a lock, but clearing the table (reset) or destroying the registry while frames may still arrive is the engine's (slice-4) responsibility to sequence, as with the collector (ADR-0013)
